...
int8_t maximum_month_temperature(int month_number, int year_number, struct sensor** arr, int arr_size);
int8_t  maximum_year_temperature(int year_number, struct sensor** arr, int arr_size);
...
int main(void)
{
    ...
    printf("maximum %d year temp = ", array_of_pointers[0]->year);
    printf("%d\n", maximum_year_temperature(array_of_pointers[0]->year, array_of_pointers, processed_sensors_number));
...
int8_t  maximum_year_temperature(int year_number, struct sensor** arr, int arr_size)
{
    int8_t year_max_temp = -100, current_month_temp;

    for (int i = 1; i <= 12; i++)
    {
        current_month_temp = maximum_month_temperature(i, year_number, arr, arr_size);

        if (current_month_temp > year_max_temp)
        {
            year_max_temp = current_month_temp;
        }
    }

    return year_max_temp;
}
